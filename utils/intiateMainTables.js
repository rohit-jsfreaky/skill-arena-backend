...
export const createFinalSchema = async () => {
  console.log("Creating final schema...");
  try {
    await pool.query(`
...
CREATE TABLE IF NOT EXISTS games (
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  access_type VARCHAR(10) NOT NULL DEFAULT 'free' CHECK (access_type IN ('free', 'pro'))
);
...
UPDATE games SET access_type = 'pro'
WHERE id IN (
  SELECT DISTINCT game_id FROM membership_games
    ...
    console.error("❌ Error creating final schema:", error);
    throw error;
  }
};
